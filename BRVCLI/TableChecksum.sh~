...
while true; do
    ...
    -h)
        ...
        printf ":--dp {Destination SQL server's password}\n\n"
				printf "USAGE:\n\n"
        printf "Backup:\n"
        printf "./TableChecksum.sh -b -u {} -p {} -d {} -t {} --dir {}\n"
				printf "./TableChecksum.sh -b -u {} -p {} -d {} --dir {}\n"
        printf "./TableChecksum.sh -b -u {} -p {} --all --dir {}\n"
        printf "fill the respective details instead of '{}'\n\n"
				printf "Restore:\n"
        printf "./TableChecksum.sh -r --du {} --dp {} -d {} -f {}\n\n"
        printf "Validate:\n"
				printf "If the backup is already restored\n"
        printf "./TableChecksum.sh -v -u {} -p {} -d {} -t {} -f{}\n"
        printf "./TableChecksum.sh -v -u {} -p {} -d {} -f{}\n"
				printf "If the backup is not yet restored\n"
        printf "./TableChecksum.sh -v -u {} -p {} -f{}\n"
        ;;
    -b)
        backup=1
        ;;
...
findChecksumTable(){
	tableChecksum=`echo "checksum table ${mysql_table}" | mysql --user=${mysql_user} --password=${mysql_password} --database=${mysql_database} |grep -v '^Table\tChecksum' `

	#if the above query failed to get results
  if [ "$?" -ne 0 ]
  then
    printf "error occurred, check if ${mysql_database} and ${mysql_table} exists \n"
    exit 1
  fi

  i=0
  for cs in $tableChecksum
  do
    if [ "$i" -eq 3 ]; then
      checksum=$checksum"\n"$cs 
    fi
...
backup_database(){
	...
	#perform backup
  mysqldump ${additional_mysqldump_params} --user=${mysql_user} --password=${mysql_password} ${mysql_database} > "$master_backup_file/${mysql_database}_backup.sql" 2>database.err 
...
backup_table(){
	...
	#perform backup
  mysqldump --user=${mysql_user} --password=${mysql_password} ${mysql_database} ${mysql_table} > "$master_backup_file/${mysql_database}_${mysql_table}_backup.sql"  2>database.err 
...
verifyChecksum(){
  ...
  master_backup_file=$file".txt"
	...
	#check for identity
	if diff "dumb.txt" $master_backup_file > /dev/null
	then
    printf "backup database is valid\n"
	else
  	printf "backup database is corrupted\n"
	fi
